...
class Sentence():
    ...
    def mark_mine(self, cell):
        ...
        """
        # Does not mark mine if mines are already known
        if self.count != len(self.cells):
    ...
    def mark_safe(self, cell):
        ...
        """
        # Does not mark safe cell if the safe cells are already known
        if self.count != 0:
            if cell in self.cells:
                self.cells.remove(cell)
...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        """
        # Marks cell as a move and safe
        self.moves_made.add(cell)
        self.mark_safe(cell)

        cells = set()

        # Gets the cells around the cell that are not know to be safe or mines
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                if (i, j) == cell:
                    continue
                elif i < 0 or i >= self.height:
                    continue
        ...
        # Adds new sentence to the knowledge base
        new_sentence = Sentence(cells, count)
        self.knowledge.append(new_sentence)

        # Updates safe and mine cells based on knowledge base
        for sentence in self.knowledge:
    ...
    def make_safe_move(self):
        ...
        """
        # Returns a safe cell that has not already been picked
        for safe_cell in self.safes.difference(self.moves_made):
            return safe_cell
